...
import { useUpdateServiceMutation } from '../../../../app/features/product/serviceApi';
...
const UpdateService = () => {
    ...
    const { data: editData } = useLoaderData();
    const [subInput, setSubInput] = useState([]);
    const [preview, setPreview] = useState({ imageLive: editData?.image });
    const [imgFile, setImgFile] = useState(null);
    const [upLoaded, setUpLoaded] = useState(null);
    const [updatedServiceData, setUpdatedServiceData] = useState(null);
    const { register, handleSubmit, formState: { errors } } = useForm({
        defaultValues: updatedServiceData || editData
    });
    const [updateService, { isLoading, isSuccess, isError, error, data }] = useUpdateServiceMutation();
    console.log(data)
    useEffect(() => {
        if (isError) {
            console.log(error)
            ...
            setUpdatedServiceData(data.data)
            setUpLoaded(null)
    ...
    const handleCreateService = async data => {
        ...
        if (imgFile) {
            ...
            if (result.data.file) {
                ...
                updateService({ id: editData._id, updatedData: { ...data, image: result.data.file } });
            } else {
                console.log(result);
            };
        } else {
            updateService({ id: editData._id, updatedData: data });
        }
    };
    return (
        <div
        ...
        >
            <h2 className='text-2xl text-center my-4'>Update your service.</h2>
            <form
                onSubmit={handleSubmit(handleCreateService)}
                className='mb-6 lg:max-w-3xl xl:max-w-5xl mx-auto'
            >
                ...
                <div className="w-full mb-4">
                    ...
                    <textarea
                        ...
                        className="w-full min-h-[130px] text-gray-800 bg-slate-200 py-2 px-3 mt-2 border focus:outline-gray-600 border-blue-500 rounded-md"
                    />
                    {errors.details?.type === 'required' && <p role="alert" className='pl-4px text-sm -mb-4'>{errors.details?.message}</p>}
                </div>
                <button type='submit' className='w-40 px-8 py-3 mx-auto block mt-3 bg-blue-700 active:outline outline-green-500 active:text-yellow-400 font-semibold text-white rounded-md hover:bg-blue-800'>{isLoading ? <SmallSpinner /> : "Update"}</button>
            </form>
        </div>
    );
};
